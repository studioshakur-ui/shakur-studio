...
import { buildAuditPrompt, buildSystemPrompt } from '../_shared/prompts.ts';
import { auditSchema } from '../_shared/schemas.ts';
import { demoAudit } from '../_shared/demo.ts';
import { AgentEnvelope, AuditAgentInput, AuditAgentOutput } from '../_shared/types.ts';
import {
  optionalString,
  parseJsonBody,
  requireLanguage,
...
async function parseInput(req: Request): Promise<AuditAgentInput> {
  const body = await parseJsonBody(req);
  return {
    subject: requireString(body, 'subject', { min: 4, max: 600 }),
    audience: optionalString(body, 'audience', { max: 240 }),
    objective: optionalString(body, 'objective', { max: 240 }),
...
Deno.serve(async (req) => {
  ...
  if (!hasOpenAIKey()) {
    const envelope: AgentEnvelope<AuditAgentOutput> = {
      agent: 'audit',
      mode: 'demo',
      language: input.language,
      result: demoAudit(input.language, input.subject)
    };
    logAgentRun({
      agent: 'audit',
      language: input.language,
  ...
  try {
    const result = await chatJson<AuditAgentOutput>({
      systemPrompt: buildSystemPrompt(input.language),
      userPrompt: buildAuditPrompt(input),
      schema: auditSchema,
      schemaName: 'audit_agent_output'
    });
    const envelope: AgentEnvelope<AuditAgentOutput> = {
      agent: 'audit',
      mode: 'live',
